...
const STATIC_VISION =
  "Venezia 2026: una città più abitabile, trasparente e capace di unire storia e futuro.";

function ProgramPage({ onBack }: { onBack: () => void }) {
  return (
      ...
      <div className="max-w-4xl mx-auto bg-white border-[8px] md:border-[16px] border-venice-dark p-8 md:p-16 shadow-2xl relative">
        ...
        <div className="space-y-8">
          <div className="markdown-body text-venice-dark selection:bg-venice-red selection:text-white">
            <ReactMarkdown>{STATIC_PROGRAM}</ReactMarkdown>
          </div>
        </div>
...
function Header({ onOpenProgram }: { onOpenProgram: () => void }) {
  return (
            ...
            {topics.length > 0 ? (
              topics.slice(0, 5).map((topic) => (
                <li key={topic.path || topic.sha} className="flex items-center gap-2 group">
                  ...
                  >
                    {String(topic.name).replace('.md', '').replaceAll('-', ' ')}
                  </a>
                </li>
              ))
            ) : (
  ...
  return (
    <div className="bg-venice-red/5 p-8 md:p-12 flex flex-col justify-between min-h-[500px]">
        ...
        {step === 'edit' ? (
          <form onSubmit={handleNext} className="space-y-4">
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className={`py-2 text-[10px] font-bold uppercase tracking-widest border border-venice-red transition-all ${
                  formData.category === 'Proposta' ? 'bg-venice-red text-white' : 'text-venice-red'
                }`}
              >
                Proposta
              </button>
              <button
                ...
                className={`py-2 text-[10px] font-bold uppercase tracking-widest border border-venice-red transition-all ${
                  formData.category === 'Segnalazione' ? 'bg-venice-red text-white' : 'text-venice-red'
                }`}
              >
                Segnalazione
              </button>
            ...
            {status === 'error' && (
              <p className="text-[10px] text-venice-red font-bold uppercase tracking-widest text-center mt-2">
                Errore di rete
              </p>
            )}
          </form>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              <label className="text-[10px] uppercase font-bold tracking-widest text-venice-red">
                Verifica Umana: Quanto fa {mathChallenge.q}?
              </label>
              <input
                type="number"
                required
      ...
      <div className="pt-8 mt-12 border-t border-venice-red/10">
        <h4 className="text-[10px] uppercase font-bold tracking-widest text-venice-red mb-4">
          Ultime istanze
        </h4>
...
function Footer({ repoInfo }: { repoInfo: any }) {
  return (
...
export default function App() {
  ...
  const [vision, setVision] = useState<string>(STATIC_VISION);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (!contextRes.ok) {
          setError('Server non raggiungibile. Verifica la configurazione su Vercel.');
  ...
  if (view === 'program') {
    return <ProgramPage onBack={() => changeView('home')} />;
  }
